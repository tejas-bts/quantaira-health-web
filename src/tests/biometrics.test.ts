...
const valuesArray = [[1649794435000, 19.8], [1649794431000, 19.9], [1649794433000, 19.9], [1649794437000, 19.8]];
const data = [{
  label: 'Temp',
...
test('Appended items should be in a sorted order', () => {
  ...
  const sortedValues = valuesArray.slice().sort((a, b) => { return a[0] - b[0]; });
...
test('Appended items should not be in a non-sorted order', () => {
  ...
  const reverseSortedValues = valuesArray.slice().sort((a, b) => { return b[0] - a[0]; });
